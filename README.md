...
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=32&pause=1000&color=6366F1&center=true&vCenter=true&width=700&lines=IT+Support+Ticketing+System;Built+for+Speed.+Designed+for+Clarity." alt="Typing SVG" />
...
<p align="center">
  A modern, production-ready internal IT helpdesk solution — engineered with a strict role-based access model, a clean React dashboard, and a robust REST API. Built as a portfolio project to demonstrate full-stack architecture, security best practices, and polished UI/UX design.
</p>
...
- 🔴 **No single source of truth** — Support teams juggle multiple inboxes and chat threads.
- 🔴 **Zero accountability** — Requests have no owner, no status, and no audit trail.
- 🔴 **Lost context** — Conversations disappear; the same issue gets reported 10 times.
- 🔴 **No prioritization** — Every request feels equally urgent because nothing is categorized.

### The Solution
- ✅ **One centralized dashboard** where every ticket has a clear owner, status, and history.
- ✅ **A strict, one-way lifecycle** (`OPEN → IN_PROGRESS → RESOLVED`) that prevents workflow regressions.
- ✅ **Role-based access** that separates what a helpdesk admin can do from what an end user can do — enforced both at the API level and in the UI.
- ✅ **A modern, responsive interface** that feels as polished as a SaaS product, making adoption frictionless for end users.

The goal was not just to build a CRUD app — it was to build a system that enforces *good process* by design, while demonstrating solid full-stack engineering fundamentals.
...
| 📄 | **Server-Side Pagination** | Efficient cursor-style pagination with full `meta` info returned on every list |
...
├── backend/     ⚙️  NestJS REST API — see backend/README.md for full details
└── frontend/    🖥  Next.js 14 Client App — see frontend/README.md for full details
```
